...
<section class="container mx-auto px-4 md:px-6 py-12 flex-grow flex items-center justify-center">
  <div class="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-4xl w-full border border-gray-100">
    
    <?php if ($top_penyakit && $top_cf > 0.01) { ?>
      <div class="border border-gray-300 rounded-lg overflow-hidden">
        
        <div class="bg-indigo-500 text-white font-bold p-3 text-lg text-center">
          Hasil Konsultasi
        </div>

        <div class="p-4 border-b border-gray-300">
            <h3 class="font-semibold mb-2 text-gray-700">Gejala Yang Dipilih:</h3>
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nilai CF User</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-900"><?= $gj['id'] ?></td>
                        <td class="px-3 py-2 text-sm text-gray-900"><?= $gj['nama'] ?></td>
                        <td class="px-3 py-2 whitespace-nowrap text-sm text-indigo-600 font-medium"><?= $gj['cf_user'] ?></td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
        ...
        <div class="p-4 bg-indigo-50 border-b-2 border-indigo-200">
            <p class="font-bold text-lg text-indigo-700 mb-2">
                Kemungkinan Hewan Terdiagnosa Penyakit: <?= $data['nama_penyakit'] ?>
            </p>
            <p class="font-bold text-xl text-orange-600">
                Tingkat Kepastian: <?= number_format($top_cf * 100, 2) ?>% 
                (<?= $cf_keterangan['keterangan'] ?? 'Tidak Terdefinisi' ?>)
            </p>
        </div>
      ...
      <div class="text-center mt-6 flex justify-between items-center">
          <button class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-medium">
              CETAK
          </button>
          
          <a href="../konsultasi.php" class="px-6 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 font-medium">
    ...
    <?php } else { ?>
      ...
      <h2 class="text-3xl font-bold mb-4 text-red-600">❌ Tidak Ada Diagnosa Kuat</h2>
      <p class="text-gray-700 mb-8 text-lg">Sistem tidak dapat menemukan penyakit dengan tingkat keyakinan yang memadai.</p>
      
      <a href="pertanyaan_diagnosa.php" class="px-8 py-3 bg-indigo-500 text-white font-semibold rounded-full hover:bg-indigo-600 transition duration-300 shadow-lg transform hover:scale-105">
          Coba Lagi
      </a>
    <?php } ?>
  </div>
